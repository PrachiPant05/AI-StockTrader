...
sys.path.insert(0, PROJECT_ROOT)  # Ensure it's the first path Python checks
...
if data is not None and not data.empty:
    ...
    if st.sidebar.button("Run Backtest"):
        with st.spinner("Running backtest..."):
            ...
            st.plotly_chart(plot_stock_data(data), width='stretch')

            st.write("### Backtest Results with Buy/Sell Signals")
            st.plotly_chart(plot_stock_with_signals(data, positions), width='stretch')
            ...
            if profit_loss:
                ...
                if cumulative_pl_values:
                    plt.figure(figsize=(12, 6))
                    color = 'g' if cumulative_pl_values[-1] >= 0 else 'r'
                    plt.plot(dates, cumulative_pl_values, marker='o', color=color)
                    ...
                    st.info("No profit/loss data available to plot.")
            else:
                st.info("No trades executed to calculate profit/loss.")
